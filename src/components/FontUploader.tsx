...
export function FontUploader({ onUpload }: FontUploaderProps) {
  ...
  const processFiles = useCallback(async (fileList: FileList) => {
    ...
    for (const file of Array.from(fileList)) {
      ...
      setFiles((prev) => {
        const existingIndex = prev.findIndex((f) => f.weight === weight && f.style === style);

        if (existingIndex >= 0) {
          const updated = [...prev];
          const existing = updated[existingIndex];
          const variants = existing.variants ?? [];
          variants.push({ name: file.name, data: buffer, format });
          updated[existingIndex] = { ...existing, variants };
          return updated;
        } else {
          return [...prev, { name: file.name, weight, style, data: buffer, format }];
        }
      });
  ...
  const handleSubmit = async () => {
    if (!fontName.trim()) {
      toast({ title: 'Font name required', description: 'Please enter a name for this font family', variant: 'destructive' });
      return;
    }
    if (files.length === 0) {
      toast({ title: 'No files selected', description: 'Please upload at least one font file', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Font uploaded', description: `${fontName} has been added to your library` });
      setFiles([]);
      setFontName('');
      setAuthor('');
      setDescription('');
